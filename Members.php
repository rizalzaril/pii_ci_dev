...
//	protected $special_admin_676 = "676";
	...
	function __construct()
	{
		...
		elseif(!$this->session->userdata('is_admin_login')){
				$this->session->set_flashdata('error', $this->access_deny_msg());SS
				redirect('admin');
				exit;
		}
	}
	...
	public function index(){
		...
		if (!in_array($this->session->userdata('type'), $akses)) {
			if ($this->session->userdata('admin_id') != $this->special_admin_676 && $this->session->userdata('admin_id') != $this->special_admin_675 && $this->session->userdata('admin_id') != $this->special_admin_782
...
//------------------------------------------------------------------------------------------ Tambahan by IP 
		...
		$data["wilayah"] = $wil ; $data["kode_bk"] = $bk ; 
		if ($bk !=""  || !empty(trim($bk))) {
			$nabk = $this->members_model->cari_nabk($bk); 
			$data["nabk"] = $nabk->name ; $data["nobk"] = $nabk->value ; 
		}		
...
//------------------------------------------------------------------------------------------------------	
	...
	public function search(){
		...
		if(!in_array($this->session->userdata('type'),$akses)){
			if($this->session->userdata('admin_id')!=$this->special_admin_676 && $this->session->userdata('admin_id')!=$this->special_admin_676 && $this->session->userdata('admin_id')!=$this->special_admin_782
			   && $this->session->userdata('admin_id')!=$this->special_admin_780 && $this->session->userdata('admin_id') != $this->special_admin_731 && $this->session->userdata('admin_id') != $this->special_admin_672
			   && $this->session->userdata('admin_id') != $this->special_admin_707)
			{
				$this->session->set_flashdata('error',$this->access_deny_msg());
		...
		if($search_name=='' && $search_email=='' && $search_kta=='' && $search_inst=='' && $search_status=='' && $search_dob =='' && $search_filter_bk=='' && $search_filter_hkk=='' && $search_filter_cab==''  
...
// ------------------------------------------------------------------------------------------------------------- Tambahan by IP		
		...
		$data["wilayah"] = $wil ; $data["kode_bk"] = $bk ; 
		if ( $bk != "" ) { $data["m_filter_cab1"] = $this->members_model->get_all_cabang_wilayah(); }
		if ($wil !="") {
			$nacab = $this->members_model->cari_nacab($wil) ; 		
			$data["nacab"] = $nacab->name ;
		}
...
EOD;
	...
	//	$akses = array("0", "8");
		...
		//	if($this->session->userdata('admin_id')!=$this->special_admin_673  && $this->session->userdata('admin_id')!=$this->special_admin_675)
			if($this->session->userdata('admin_id')!=$this->special_admin_673  && $this->session->userdata('admin_id')!=$this->special_admin_782
	...
	//	$akses = array("0", "8");
		$akses = array("0", "8", "9","1");
		if(!in_array($this->session->userdata('type'),$akses)){
			if($this->session->userdata('admin_id')!=$this->special_admin_673  && $this->session->userdata('admin_id')!=$this->special_admin_675  && $this->session->userdata('admin_id')!=$this->special_admin_782
			    && $this->session->userdata('admin_id')!=$this->special_admin_780 && $this->session->userdata('admin_id') != $this->special_admin_731 && $this->session->userdata('admin_id') != $this->special_admin_672)
			{
				$this->session->set_flashdata('error',$this->access_deny_msg());
				redirect('admin/dashboard');
				...
				if(isset($check1[0]))
				{
					...
					if(isset($check_transfer[0])){
						...
						$this->main_mod->update(
							...
							array(
								'vnv_status'=>1,'modifieddate'=>date('Y-m-d H:i:s'),
								'modifiedby'=>$this->session->userdata('admin_id')
								//,'is_upload_mandiri'=>1
							)
						);
					...
					if($checkx[0]->is_upload_mandiri == '0'){
						...
						if($checkx[0]->pay_type=='4') {
							...
							if(isset($check_faip->keputusan)){
								...
								else if($check_faip->keputusan=="IPU" ||
...
public function fungsiUploadGambar()
{
    $no_kta = $_POST['id_f'] ;
     ...
     $id_userid = $faip->user_id ; $id_table = $faip->user_id ; $nama = $faip->nama ; $faip_idd = $faip->id ;
...
public function fungsiUpload_estri()
{
    $no_kta = $_POST['id_f'] ;
     ...
     $userid = $faip->person_id ;  $nama = $faip->add_name ; 
...
//---------------------------------------------------------------------------------------------------------------- Tambahan by Ipur 08-06-2025
	...
	public function update_acpe()
	{
		...
		$this->load->model('simpan_model') ;
   		 ...
   		 $this->simpan_model->update_data_acpe($id, $data_update_acpe); $data['selesai'] = 'y' ;
...
//------------------------------------------------------------------------------------------------------------------------------------
	...
	function ajax_update_bk()
	{
		...
		$status = array();
		if(is_debug()){ $status[] = $this->debug_msg; }
		...
		$this->load->model('main_mod');
			...
			try {
				...
				if(is_debug()){ $status[] = "SUCCESS: Params validation"; }
				...
				if(is_debug()){ $status[] = "SUCCESS: Get existing user based on user_id: {$user_id}"; }
				...
				if ( empty($user->code_bk_hkk) == FALSE || ltrim($user->code_bk_hkk,'0') !== '') {
					...
					if ( ltrim($user->code_bk_hkk,'0') == $bk) {
						$this->_rest_response(FALSE, '401021',
							"Current user's BK is the same as target BK, bk_id: {$user->code_bk_hkk}", REST_Controller::HTTP_BAD_REQUEST
						);
					}
					if(is_debug()){ $status[] = "SUCCESS: Checked, target BK is different with current BK, bk_id: {$user->code_bk_hkk}"; }
					...
					if ( $result->num_rows() > 0) {
						if(is_debug()){ $status[] = "FAILED: Still has active FAIP in the previous BK"; }
						$this->_rest_response(FALSE, '401021',
					...
					if(is_debug()){ $status[] = "SUCCESS: Checked, no active FAIP with the existing BK"; }
					...
					if ( $result->num_rows() > 0) {
						if(is_debug()){ $status[] = "WARNING: Still has pending payment/VA (in user_transfer)"; }
					...
					if(is_debug()){ $status[] = "SUCCESS: Checked, no pending payment/VA (in user_transfer)"; }
				...
				if(is_debug()){ $status[] = "SUCCESS: Update table: members, person_id: {$user_id} "; }
...
//------------------------------------------------------------------------------------------------------------------------------------------
				...
				if(is_debug()){ $status[] = "SUCCESS: Update table: user_profiles, user_id: {$user_id}"; }
				...
				if(is_debug()){ $status[] = "SUCCESS: Insert into log_status_keanggotaan, id: {$log_id}"; }

				$this->_rest_response(TRUE, '200101',
...
//			if($this->session->userdata('admin_id')!=$this->special_admin_676)
	...
	public function search_pi(){
		...
		if(!in_array($this->session->userdata('type'),$akses)){
		//	if($this->session->userdata('admin_id')!=$this->special_admin_676)
			if( $this->session->userdata('admin_id')!=$this->special_admin_782 || 
	...
	function ajax_faip_setip()
	{
		...
		$ip_type = $this->input->post('ip_type')<>null?$this->input->post('ip_type'):"";
		//$tgl_sk = $this->input->post('tgl_sk')<>null?$this->input->post('tgl_sk'):"";
		$from = $this->input->post('from')<>null?$this->input->post('from'):"";			$starty = date('Y-m-d' , strtotime($from)) ;
		$stri_kp = $this->input->post('stri_kp')<>null?$this->input->post('stri_kp'):"";
		//$until = $this->input->post('until')<>null?$this->input->post('until'):"";
		$no_ip = $this->input->post('no_ip')<>null?$this->input->post('no_ip'):"";
		$no_seri = $this->input->post('no_seri')<>null?$this->input->post('no_seri'):"";
...
//---------------------------------------------------------------------------------------------------------------------------------------
	...
	{
		//
		if ($this->session->userdata('type') != ADMIN_TYPE_SUPERADMIN
...
//		}
	...
	function ajax_update_faip_bk()
	{
		...
		try{
			...
			if(is_debug()){ $status[] = "SUCCESS: Get existing faip based on faip_id: {$faip_id}"; }
				...
				if(is_debug()){ $status[] = "SUCCESS: Dump data asesor: ". print_r($result, TRUE); }

				//Delete data asesor
				$this->db->where('faip_id', $faip_id)->delete('asesor_faip');
				if(is_debug()){ $status[] = "SUCCESS: Delete asesor_faip, faip_id: {$faip_id}"; }
				...
				if(is_debug()){ $status[] = "SUCCESS: Update user_faip, faip_id: {$faip_id}"; }
				...
				if(is_debug()){ $status[] = "SUCCESS: Insert log_status_faip, id: {$logfaip_id}"; }
			...
			if(isset($check[0]))
			{
				...
				else if ($status=="9")
				{
					...
					try{
						...
						if($keputusan=="Belum memenuhi persyaratan untuk sertifikasi IPP" ||
	...
	public function faipview_assessor_result()
	{
		...
		$faipWithCompetency_list = array(
			'user_faip_13', 'user_faip_14', 'user_faip_15', 'user_faip_16', 'user_faip_21',
			'user_faip_22', 'user_faip_3', 'user_faip_4', 'user_faip_51', 'user_faip_52',
			'user_faip_53', 'user_faip_54', 'user_faip_6'
		);
...
//		$this->load->model('members_model');
                $dibuat = $this->members_model->ambil_create_date($id)  ; $dibuatnya = $dibuat->createddate ;
...
FROM `m_bk` a WHERE 1")->result();
		...
		$rowCount = 2;
		$muda_aktif = 0;$biasa_aktif = 0;
		$luar_biasa_aktif = 0;$kehormatan_aktif = 0;
		$muda_tidak_aktif = 0;$biasa_tidak_aktif = 0;
		$luar_biasa_tidak_aktif = 0;$kehormatan_tidak_aktif = 0;
		$total = 0;
		foreach($rsl as $val){
			$objPHPExcel->getActiveSheet()->SetCellValue('A'.$rowCount, $val->value);
			$objPHPExcel->getActiveSheet()->SetCellValue('B'.$rowCount, $val->name);
...
FROM `m_bk` a WHERE 1")->result();
	...
	public function export_lap_cab_1(){
		 ...
		 $data["data_wil"] = $data_wil ; $data["kowil"] = $kowil ; $data["nawil"] = $nacab ; $data["jml"] = $jml ;
...
FROM `m_cab` a WHERE length(value)>2")->result();//and (person_id<>1 and person_id<>35448)
	...
	public function lap_cab_1(){
		 ...
		 $jml = (trim($this->input->post('jml',true)))? trim($this->input->post('jml',true)) : '';	
		 $data_wil = $this->members_model->ambil_data_cabang_per_wilayah($kowil) ;
		 $data["data_wil"] = $data_wil ; $data["kowil"] = $kowil ; $data["nacab"] = $nacab ; $data["jml"] = $jml ;
	...
	public function lap_cab(){
		...
		foreach($jumlah as $row) {
		        $id = $row->id	; $fiel = 'wil_id' ;
			$jcabang = $this->members_model->hitung_jumlah_cabangnya($row->kowil) ; $jcabang = $jcabang - 1;
...
FROM `m_bk` a WHERE 1=1 $f_bk")->result();
		...
		$rowCount = 2;
		$total_jml_bk = 0;$total_jml_fin = 0;$total_jml_lski = 0;
		$total[0] = 0;
		$total[1] = 0;
		$total[2] = 0;
		$total[3] = 0;
...
FROM `m_bk` a WHERE 1=1 $f_bk")->result();
	...
	public function export_stri(){
		...
		$rowCount = 2;
		$aktif = 0;$tidakaktif = 0;$total = 0;$ipp = 0;$ipm = 0;$ipu = 0;$per = 0;
		foreach($rsl as $val){
			$objPHPExcel->getActiveSheet()->SetCellValue('A'.$rowCount, $val->value);
			$objPHPExcel->getActiveSheet()->SetCellValue('B'.$rowCount, $val->name);
					...
					else if ($status=="9")
					{
						...
						try{
							...
							else
							{
								...
								$wajib1_score =0;$wajib2_score =0;$wajib3_score =0;$wajib4_score =0;$wajib5_score =0;
								$count=0;
								foreach($check2 as $val2){
									$wajib1_score = $wajib1_score+$val2->wajib1_score;
									$wajib2_score = $wajib2_score+$val2->wajib2_score;
...
EOD;
	...
	function simpan_edit_estri(){
	     ...
	     $data['id_table'] = $id ; $data['kta'] = $kta ; $data['nama'] = $nama ;
