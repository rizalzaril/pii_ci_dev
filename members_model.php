...
*/
	...
	public function record_count_faip($table_name)
	{
		...
		if (
			!isAdminLSKI() && !isAdminMajelisAll() && !isAdminBKWilayahKolektif() && !isAdminKolektifRO()
...
user_pkb.is_upgrade_paid,user_pkb.createddate,user_pkb.createdby,user_pkb.modifieddate,user_pkb.modifiedby,
	...
	public function filter_ip_test($search, $limit, $start, $order_field, $order_ascdesc, $column, $filter)
	{
	...
	public function filter_setstri($search, $limit, $start, $order_field, $order_ascdesc, $column, $filter)
	{
		...
		foreach ($filter as $v => $w) {
			if ($v == 'status') {
				if ($w == '1') {
					$this->db->where('user_transfer.rel_id<>0', null);
				} else if ($w == '2') {
					$this->db->where('user_transfer.rel_id', 0);
				}
			} else if ($v == 'cab') {
	...
	public function filter_approval($search, $limit, $start, $order_field, $order_ascdesc, $column, $filter)
	{
		...
		foreach ($filter as $v => $w) {
			if ($v == 'status') {
				if ($w == '1') {
					$this->db->where('user_approval.status', 'Approved');
				} else if ($w == '0') {
					$this->db->where('user_approval.status', 'Waiting for Approval');
				} else if ($w == '2') {
					$this->db->where('user_approval.status', 'Rejected');
				}
			} else if ($v == 'cab') {
	...
	*/
	...
	private function _get_datatables_query_old($is_kolektif = false, $bk = "", $wil = "")
	{
		$this->db->select('
	      users.id as ID,
	      users.username as no_kta,
	  ...
	  ');
		...
		if ($is_kolektif) {
			...
			$this->db->group_start();
			if ($str != '') $this->db->where("up.user_id IN (SELECT person_id FROM members WHERE $str)", null, false);
			if ($str2 != '') $this->db->or_where("up.user_id IN (SELECT person_id FROM members WHERE $str2)", null, false);
			$this->db->group_end();
		}
	...
	private function _get_datatables_query($is_kolektif = false, $bk = "", $wil = "")
	{
        ...
        ut_last.bukti as payfile,
        ut_last.status as paystatus,
        ut_last.iuranpangkal,
        ut_last.iurantahunan,
        ut_last.sukarelaanggota,
        ut_last.sukarelagedung,
        ut_last.sukarelaperpus,
        ut_last.sukarelaceps,
        ut_last.sukarelatotal,
        ut_last.id as payid,
        ut_last.vnv_status,
        ut_last.remark,
        ut_last.createddate as tgl_pengajuan,

				(SELECT x.id FROM user_transfer x WHERE x.pay_type=2 AND x.user_id = up.user_id ORDER BY x.createddate DESC LIMIT 1) as id_pay_cek,

	      (SELECT y.id_pay FROM user_transfer x JOIN log_her_kta y ON y.id_pay=x.id WHERE x.pay_type=2 AND x.user_id=up.user_id ORDER BY x.createddate DESC LIMIT 1) as id_pay,

	      (SELECT from_date FROM user_transfer x JOIN log_her_kta y ON y.id_pay=x.id WHERE x.pay_type=2 AND x.user_id=up.user_id ORDER BY x.createddate DESC LIMIT 1) as plan_from_date,

	      (SELECT thru_date FROM user_transfer x JOIN log_her_kta y ON y.id_pay=x.id WHERE x.pay_type=2 AND x.user_id=up.user_id ORDER BY x.createddate DESC LIMIT 1) as plan_thru_date,

				(select sukarelatotal from user_transfer where user_transfer.pay_type=2 and up.user_id=user_id order by createddate desc limit 1) as paysukarelatotal,
    ...
    ');
		...
		// transfer terakhir
		$this->db->join("(SELECT t1.* FROM user_transfer t1
        INNER JOIN (
            SELECT user_id, MAX(createddate) as last_created
            FROM user_transfer
            WHERE pay_type=2
		...
		// cert terakhir
		$this->db->join("(SELECT c1.* FROM user_cert c1
        INNER JOIN (
            SELECT user_id, MAX(endyear) as last_year
            FROM user_cert
            WHERE status=2
		...
		// join members (langsung, bukan subquery)
		$this->db->join('members m', 'm.person_id = users.id', 'left');

		// wajib punya transfer
		$this->db->where("EXISTS (SELECT 1 FROM user_transfer ut WHERE ut.pay_type=2 AND ut.user_id=users.id)", null, false);

		// filter kolektif
		if ($is_kolektif) {
			if (!empty($bk)) {
				$this->db->group_start();
				foreach ((array)$bk as $val) {
					$this->db->or_like("LPAD(m.code_bk_hkk,2,'0')", $val, 'after');
				}
				$this->db->group_end();
			}
			if (!empty($wil)) {
				$this->db->group_start();
				foreach ((array)$wil as $val) {
					$this->db->or_like("LPAD(m.code_wilayah,4,'0')", $val, 'after');
				}
				$this->db->group_end();
			}
		}
		...
		if (!empty($_POST['search']['value'])) {
			$this->db->group_start();
			$this->db->like('up.firstname', $_POST['search']['value']);
			$this->db->or_like('up.lastname', $_POST['search']['value']);
			$this->db->or_like('users.username', $_POST['search']['value']);
			$this->db->group_end();
		}
	...
	public function get_her_kta($length, $start, $is_kolektif = false, $bk = "", $wil = "")
	{
	...
	public function count_filtered_her($is_kolektif = false, $bk = "", $wil = "")
	{
		...
		if ($is_kolektif) {
			...
			if (!empty($bk)) {
				...
				$conditions[] = 'up.user_id IN (SELECT person_id FROM members WHERE ' . implode(' OR ', $bk_cond) . ')';
			}
			...
			if (!empty($wil)) {
				...
				$conditions[] = 'up.user_id IN (SELECT person_id FROM members WHERE ' . implode(' OR ', $wil_cond) . ')';
			}
