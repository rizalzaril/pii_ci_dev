										...
										$profileFields = [
											...
											'ID Card' => function ($d) {
												if (!empty($d['idcard'])) {
													...
													$downloadUrl = base_url('admin/file_access/download_idcard/' . $d['id_file']);
										...
										<?php if (!empty($detail_aer['experiences'])): ?>
											<?php foreach ($detail_aer['experiences'] as $exp): ?>
												<tr class="text-dark">
													...
													<td>
														<?php
														$downloadExpUrl = base_url('admin/file_access/exp_file/' . $exp['attachment']);
														if (!empty($exp['attachment'])): ?>
															<a href="<?= $downloadExpUrl ?>"
											...
											<?php foreach ($detail_aer['educations'] as $edu): ?>
												<?php if (isset($edu['status']) && $edu['status'] == 1): ?>
													...
													<tr class="text-dark">
														...
														<td>
															<?php
															$downloadEduUrl = base_url('admin/file_access/edu_file/' . $edu['attachment']);
															if (!empty($edu['attachment'])): ?>
																<a href="<?= $downloadEduUrl ?>"
																	class="btn btn-xs btn-danger" target="_blank">
																	<i class="fa fa-arrow-down"></i> PDF
																</a>
															<?php else: ?>
											...
											<?php foreach ($detail_aer['certifications'] as $cert): ?>
												<?php if (isset($cert['status']) && $cert['status'] == 1): ?>
													...
													<tr class="text-dark">
														...
														<td>
															<?php
															$downloadCertUrl = base_url('admin/file_access/cert_file/' . $cert['attachment']);
															if (!empty($cert['attachment'])): ?>
																<a href="<?= $downloadCertUrl ?>"
																	class="btn btn-xs btn-danger" target="_blank">
																	<i class="fa fa-arrow-down"></i> PDF
																</a>
															<?php else: ?>
