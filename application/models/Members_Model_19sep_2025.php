...
class Members_Model extends CI_Model
{
  ...
  public function get_all_members_v2_OLD_2($per_page, $page, $bk, $wil, $is_kolektif)
  {
...
(select createddate from user_transfer where d.ID=user_id and (pay_type=0 or pay_type=1) order by createddate desc limit 1) as tgl_pengajuan", false)
  ...
  public function get_all_her_kta($per_page, $page, $bk, $wil, $is_kolektif)
  {
    ...
    $this->db->select('
    ut.id as payid,
    ut.createddate as tgl_pengajuan,
    users.id as ID, 
    users.username as no_kta, 
    users.email, 
    up.firstname, 
    ...
    ut.atasnama as payname,
    ut.tgl as paydate,
    ut.description paydesc,
    ut.bukti as payfile,
    ut.status as paystatus,
    ut.iuranpangkal as payiuranpangkal,
    ut.iurantahunan as payiurantahunan,
    ut.sukarelaanggota as paysukarelaanggota,
    ut.sukarelagedung as paysukarelagedung,
    ut.sukarelaperpus as paysukarelaperpus,
    ut.sukarelaceps as paysukarelaceps,
    ut.sukarelatotal as paysukarelatotal,

    uc.lic_num as sip_lic_num,
    uc.startyear as sip_startyear,
    uc.endyear as sip_endyear,
				
    lh.id_pay  as id_pay, 
    lh.from_date as plan_from_date, 
    lh.thru_date as plan_thru_date, 
				
    ut.id as id_pay_cek, 
    ut.vnv_status as vnv_status,
    ut.remark as remark,
				
    m.code_wilayah as cab,
    m.code_bk_hkk as bk,
    m.from_date  as from_date,
    m.thru_date as thru_date
');
    $this->db->from('(SELECT MAX(id) as last_id FROM user_transfer WHERE pay_type=2 GROUP BY user_id) t');
    $this->db->join('user_transfer ut', 'ut.id = t.last_id', 'left');
    $this->db->join('log_her_kta lh', 'ut.id = lh.id_pay', 'left');
    $this->db->join('user_profiles up', 'ut.user_id = up.user_id', 'left');
    $this->db->join('users', 'up.user_id = users.id', 'left');
    $this->db->join('members m', 'm.person_id = users.id', 'left');
    $this->db->join(
      '(SELECT user_id, lic_num, startyear, MAX(endyear) as endyear 
                  FROM user_cert 
                  WHERE status=2 
                  GROUP BY user_id) uc',
      'ut.user_id = uc.user_id',
      'left'
    );
    $this->db->where('(users.username <> "" OR users.username IS NOT NULL)');
