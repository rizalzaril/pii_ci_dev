...
class Members extends CI_Controller
{
	...
	private function status_payment($row)
	{
		$status = '';
		$class_label = 'danger';
		...
		return '<a onClick="update_status(' . $row->ID . ');" href="javascript:;" id="sts_' . $row->ID . '">
                <span class="label label-' . $class_label . '">' . $status . '</span>
            </a>';
	}

	private function status_vnv($row)
	{
		$status = '';
		$class_label = '';
		...
		if ($row->vnv_status == '1') {
			...
			$class_label = 'danger';
			$status = 'Dokumen Not Valid<br>' . $row->remark;
		...
		return ($status != '')
			? '<a href="javascript:;"><span class="label label-' . $class_label . '">' . $status . '</span></a>'
			: '';
	}
	...
	/**
	 * GET HER Members JSON for serverside add by Rizal
	 */
	...
	public function get_her()
	{
		...
		if (strpos($bk, ',') !== false) $bk = explode(",", $bk);
		if (strpos($wil, ',') !== false) $wil = explode(",", $wil);

		$is_kolektif = in_array($this->session->userdata('type'), ["11", "15"]);
		...
		foreach ($list as $row) {
			...
			foreach ($docs as $field => $label) {
				if (!empty($row->$field)) {
					$dokumen .= '<a target="_blank" href="' . base_url('assets/uploads/' . $row->$field) . '">- ' . $label . '</a><br>';
				}
			}
			...
			$data[] = array(
				...
				"dob"            => $row->dob,
				"sip"            => $row->sip_lic_num ? $row->sip_lic_num . '<br>(' . $row->sip_startyear . ' sampai ' . $row->sip_endyear . ')' : '',
				"warga_asing"    => $row->warga_asing == 0 ? 'WNI' : 'WNA',
				"dokumen"        => $dokumen,
...
EOD;
	...
	//  Proteksi file SKIP By Rizal Liyan Syah
	public function skip_file($filename = '')
	{
		...
		if (!in_array($type, $akses)) {
			...
			// Path file di server
			$filepath = FCPATH . 'assets/SKIP/' . $filename;
	...
	//  Proteksi file STRI By Rizal Liyan Syah
	public function stri_file($filename = '')
	{
		// Daftar role/akses yang diizinkan
		$akses = array("0", "1", "2", "9", "10", "11", "12", "14", "15");
		...
		if (!in_array($type, $akses)) {
			...
			// Path file di server
			$filepath = FCPATH . 'assets/ESTRI/' . $filename;
