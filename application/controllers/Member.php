...
//--------------------------------------------- Tambahan by Ipur Tgl 05-06-2025 ------------------------------------------------------------
		 ...
		 if (isset($dibuat)) {
			...
			$this->load->model('faip_model') ; $nokta = $this->faip_model->golek_user_id_sertifikat($id) ;
			$data['nokta'] = $nokta->no_kta ; // Tambahan by Ipur Tgl.2025-06-16		 	
		 }else{
		  	$data['dibuatnya'] = '' ; $data['nokta'] = '' ; // Tambahan by Ipur Tgl.2025-06-16
		 } 	
...
EOD;
	...
	function download_stri_ttd()
	{
		...
		$stri=$this->main_mod->msrwhere('members_certificate',array('person_id'=>$id,'id'=>$cert_id),'id','desc')->result();	
                $dibuat = $this->members_model->ambil_create_date($id)  ; $dibuatnya = $dibuat->createddate ; // Tambahanby Ipur 
